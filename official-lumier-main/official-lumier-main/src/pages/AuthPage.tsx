...
const AuthPage = () => {
  ...
  // useEffect для таймера обратного отсчета
  useEffect(() => {
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      if (data.success || data.status === "ok" || data.message === "Success") {
        setResendTimer(60);
        setCanResend(false);
        setLoginCode(""); // Очищаем поле кода
        toast({
          title: "Код отправлен повторно",
          description: "Мы отправили новый код на ваш номер",
        });
      } else {
        throw new Error(data.message || "Ошибка при отправке кода");
      }
            ...
            <form onSubmit={handleLogin} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-12 h-14 bg-white/5 border-white/10 text-white placeholder:text-gray-600 rounded-2xl focus:ring-blue-500/20 focus:border-blue-500/50 transition-all text-lg font-bold"
                    required
                    autoFocus
                    maxLength={4}
                  />
              ...
              <div className="text-center">
                <button
                  ...
                  className={`text-sm transition-colors ${canResend && !isRequesting
