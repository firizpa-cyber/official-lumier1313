...
export const heroSlides: HeroSlide[] = allRawMovies.slice(0, 5).map(m => ({
  id: m.id.toString(),
  title: m.title,
  subtitle: "Премьера",
  description: m.description,
...
export const popularMovies: Movie[] = allRawMovies
  .sort((a, b) => parseFloat(b.rating) - parseFloat(a.rating))
  .slice(0, 12)
  .map(transformMovie);

export const freeMovies: Movie[] = allRawMovies
  .slice(12, 24)
  .map(transformMovie);

export const kidsMovies: Movie[] = allRawMovies
  .map(transformMovie)
  .filter(m => {
...
export const kids0Plus: Movie[] = allRawMovies
  .map(transformMovie)
  .filter(m => m.ageRating === "0+")
  .slice(0, 12);

export const kids6Plus: Movie[] = allRawMovies
  .map(transformMovie)
  .filter(m => m.ageRating === "6+" || m.ageRating === "0+")
  .slice(0, 12);

export const kids12Plus: Movie[] = allRawMovies
  .map(transformMovie)
